...
int main(void) {
    ...
    bind_result = bind(base_socket, (struct sockaddr *)&server_addr , sizeof(server_addr))
    if (bind_result == -1) {
        return 0;
    }
    ...
    listen_result = listen(bind_result, 10);
    if (listen_result == -1) {
        return 0;
    }
    ...
    while(1) {
        ...
        client_socket = accept(base_socket, (struct sockaddr *)&client_addr, sizeof(struct client_addr)) 
        if (client_socket == -1) {
            // 수락 실패하면 다음 요청을 받자
